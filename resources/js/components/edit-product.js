class EditProductImageManager {
    constructor() {
        ...
        this.acceptedTypes = ['image/jpeg', 'image/jpg', 'image/png', 'image/gif'];
    ...
    getExistingImages() {
        ...
        const items = existingContainer.querySelectorAll('.existing-image-item input[type="hidden"]');
    ...
    handleFiles(files) {
        ...
        if (totalImages > this.maxFiles) {
            this.showNotification(`Maksimal ${this.maxFiles} gambar total!`, 'warning');
            return;
        }
        ...
        this.showNotification(`${validFiles.length} gambar baru ditambahkan!`, 'success');
    }
    ...
    updateNewPreview() {
        ...
        this.newFiles.forEach((file, index) => {
            ...
            reader.onload = (e) => {
                ...
                imageDiv.innerHTML = `
                    <img src="${e.target.result}" class="image-preview-img" alt="New ${index + 1}">
                    <div class="drag-handle">
                        <i class="fas fa-arrows-alt"></i>
                    </div>
                    <div class="order-indicator">${this.existingImages.length + index + 1}</div>
    ...
    updateNewOrder() {
        const items = Array.from(document.querySelectorAll('#sortableNewImages .image-preview-item'));
    ...
    updateExistingDisplay() {
        ...
        items.forEach((item, index) => {
            ...
            if (removeBtn) {
                removeBtn.setAttribute('onclick', `removeExistingImage(${index})`);
            }
        });
    }
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (document.getElementById('existingImages') || document.getElementById('dropZone')) {
